...
databaseChangeLog(logicalFilePath: 'script/db/hsgp_service_route.groovy') {
    ...
    // path唯一索引
    changeSet(author: "bojiangzhou", id: "2019-05-14-hsgp_service_route") {
        def weight = 1
        if (helper.isSqlServer()) {
            weight = 2
        } else if (helper.isOracle()) {
        ...
        dropUniqueConstraint(tableName: "hsgp_service_route", constraintName: "hsgp_service_route_u1")
        addUniqueConstraint(columnNames: "product_id,product_env_id,path", tableName: "hsgp_service_route", constraintName: "hsgp_service_route_u1")
        modifyDataType (columnName: 'service_code', newDataType: "varchar(" + 60 * weight + ")", tableName: 'hsgp_service_route')
    }
}